#!/bin/bash
# =============================================================================
#  ap-slot-new.sh — creates a new SSID slot (radio + subnet + tunnel seat)
#    sudo ap-slot-new.sh <slot> <interface> <5|2.4> [--ssid NAME] [--psk PASSWORD] [--channel N]
#                        [--country XX] [--net 10.99.0] [--desc "description"]
...
SLOT=${1:-}; AP_IF=${2:-}; BAND=${3:-}; shift 3 2>/dev/null || die "usage: ap-slot-new.sh <slot> <interface> <5|2.4> [options]"
...
SSID=""; PSK=""; CHANNEL=""; COUNTRY=""; NET=""; DESC=""
...
echo "== $SLOT: $AP_IF, $BAND GHz channel $CHANNEL, SSID '$SSID', subnet $NET.0/24, wg$IDX / table $TABLE =="
install -d -m 0755 "$S"
sed -e "s|__SLOT__|$SLOT|g; s|__DESC__|$DESC|g; s|__AP_IF__|$AP_IF|g; s|__NET__|$NET|g; s|__IDX__|$IDX|g" \
    -e "s|__TABLE__|$TABLE|g; s|__PRIO0__|$P0|g; s|__PRIO1__|$P1|g; s|__PRIO2__|$P2|g" "$T/slot.env" > "$S/$SLOT.env"
# WG_SRC placeholder 127.0.0.1: upstream DNS CANNOT leave until a profile is assigned (fail-closed); activation writes the real address.
...
systemctl enable "wg-quick@wg$IDX.service" >/dev/null 2>&1 || true
...
echo "SSID:     $SSID"
echo "PASSWORD: $PSK"
echo "No tunnel yet: clients of this SSID CANNOT reach the internet until a profile is assigned (kill switch)."
